...
// width from 172 (--ov-rest-w) to ~300 (--ov-objective-w for rotating prompts)
// and expands from center, so the window must fit the widest state plus slack.
...
// Post-transcription stack: larger screen-share pill above the compact status
// pill. Keep in sync with --ov-share-* / --ov-stack-gap / --ov-base-h in CSS.
// Wide enough for the Steel preview (720) and the four-button completion row.
const OVERLAY_RESULT_WIDTH: f64 = 760.0;
// Share preview + extraction card + status + action pill + gaps - keep in sync with CSS.
const OVERLAY_RESULT_HEIGHT: f64 = 760.0;

/// Bumped to cancel a pending result→complete transition when a new overlay
/// session starts or the overlay is dismissed early.
static RESULT_HIDE_GENERATION: AtomicU64 = AtomicU64::new(0);

/// Overlay window size (logical) for a given UI state.
fn overlay_dimensions(state: &str) -> (f64, f64) {
    match state {
        "streaming" => (OVERLAY_STREAM_WIDTH, OVERLAY_STREAM_HEIGHT),
        "result" | "complete" => (OVERLAY_RESULT_WIDTH, OVERLAY_RESULT_HEIGHT),
        _ => (OVERLAY_WIDTH, OVERLAY_HEIGHT),
    }
}

fn invalidate_pending_result_hide() {
    RESULT_HIDE_GENERATION.fetch_add(1, Ordering::Relaxed);
    crate::agent_bridge::cancel_agent();
}
...
// Breathing room above the taskbar / panel. Positioning uses the monitor
// work area, so this is a small gap - not a guess at taskbar height.
#[cfg(any(target_os = "windows", target_os = "linux"))]
...
fn calculate_overlay_position(
    ...
    let y = match settings.overlay_position {
        ...
        OverlayPosition::Bottom => {
            // Anchor to the work area so we sit just above the taskbar / dock /
            // panel instead of guessing their height from the full screen size.
            // work_area.position is in the same global space as monitor.position.
...
pub(crate) fn show_overlay_state(app_handle: &AppHandle, state: &str) {
    ...
    // A new session cancels any pending result→complete timer. Staying on
    // result/complete itself must not invalidate that transition.
    if state != "result" && state != "complete" {
        invalidate_pending_result_hide();
    }
    ...
    if let Some(overlay_window) = app_handle.get_webview_window("recording_overlay") {
        ...
        // Re-apply pin/click-through if the user locked Handy mid-run.
        crate::handy_lock::apply_to_window(&overlay_window);
...
/// Shows the live agent/screen-share UI and starts the isolated-browser
/// investigation against `objective` (usually the just-pasted transcription).
/// When the agent finishes, transitions to the completion screen.
/// Cancelled if a new recording starts or the overlay is dismissed early.
pub fn show_result_overlay_then_hide(app_handle: &AppHandle, objective: String) {
    let settings = settings::get_settings(app_handle);
    if settings.overlay_style == OverlayStyle::None {
    ...
    // Invalidate any prior result session without killing the agent we're about
    // to start (cancel_agent is called inside start_investigation_agent).
    RESULT_HIDE_GENERATION.fetch_add(1, Ordering::Relaxed);
    show_overlay_state(app_handle, "result");
    crate::agent_bridge::start_investigation_agent(app_handle, objective);
}
