...
use crate::utils::{
    self, show_processing_overlay, show_recording_overlay, show_result_overlay_then_hide,
                        ...
                        Ok(transcription) => {
                            ...
                            if processed.final_text.is_empty() {
                                ...
                                ah.run_on_main_thread(move || {
                                    ...
                                    let objective = final_text.clone();
                                    ...
                                    // Paste done → isolated browser investigates the
                                    // spoken objective; live frames fill screen-share.
                                    show_result_overlay_then_hide(&ah_clone, objective);
                                    change_tray_icon(&ah_clone, TrayIconState::Idle);
                                })
                                .unwrap_or_else(|e| {
...
/// Dim Handy UI and ignore cursor events so the desktop underneath stays usable.
struct LockHandyAction;

impl ShortcutAction for LockHandyAction {
    fn start(&self, app: &AppHandle, _binding_id: &str, _shortcut_str: &str) {
        let locked = crate::handy_lock::toggle(app);
        debug!("lock_handy toggled -> locked={locked}");
    }
...
pub static ACTION_MAP: Lazy<HashMap<String, Arc<dyn ShortcutAction>>> = Lazy::new(|| {
    ...
    map.insert(
        "lock_handy".to_string(),
        Arc::new(LockHandyAction) as Arc<dyn ShortcutAction>,
    );
    map.insert(
        "test".to_string(),
